...
async function main() {
  ...
  (async () => {
    const eldaPoolContractIndex = await question('What is elda pool contract index?');
    ...
    await eldaPoolManager.updatePoolList(Number(eldaPoolContractIndex), String(eldaPoolContractAddress));
    rl.close()
  })()
}
